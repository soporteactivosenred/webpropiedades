...
import { Share2, Facebook, Mail, Link2, Check } from 'lucide-react';
...
export function ShareButtons({ url, title, description = '' }: ShareButtonsProps) {
  ...
  const handleShareClick = (link: (typeof shareLinks)[0]) => {
    ...
    window.open(link.href, '_blank', 'noopener,noreferrer,width=620,height=500');
    setOpen(false);
  };
  ...
  return (
    <>
      ...
      {/* Menú fixed — siempre encima de todo */}
      {open && (
        <div
          ref={menuRef}
          style={{
