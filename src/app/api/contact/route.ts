...
export async function POST(request: Request) {
  try {
    ...
    if (adminError) {
      console.error('Error enviando email a administración:', adminError);
    }
    ...
    const { data: leadEmailData, error: leadError } = await resend.emails.send({
      from: fromAddress,
      to: [email],
      subject: leadSubject,
    ...
    if (leadError) {
      console.error('Error enviando email de confirmación al lead:', leadError);
    }
