...
export async function POST(request: Request) {
  try {
    ...
    if (propertyCode || propertyTitle) {
      adminSubject = `[Consulta ${propertyCode || 'Propiedad'}] ${propertyTitle || name}`;
    ...
    const propertyInfoHtml = (propertyTitle || propertyCode || propertyUrl) ? `
      <div style="background-color: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 8px; padding: 16px; margin: 16px 0;">
        <p style="margin: 0 0 8px 0; font-size: 14px; font-weight: bold; color: #0f172a;">🏠 Propiedad Consultada:</p>
        ${propertyTitle ? `<p style="margin: 4px 0; font-size: 14px; color: #1e293b;"><strong>Título:</strong> ${propertyTitle}</p>` : ''}
        ${propertyCode ? `<p style="margin: 6px 0; font-size: 14px; color: #1e293b;"><strong>Código de Referencia:</strong> <span style="background-color: #0f172a; color: #ffffff; padding: 3px 10px; border-radius: 4px; font-family: monospace; font-weight: bold; font-size: 13px;">${propertyCode}</span></p>` : ''}
        ${propertyUrl ? `<p style="margin: 8px 0 0 0; font-size: 14px;"><a href="${propertyUrl}" target="_blank" style="color: #2563eb; font-weight: bold; text-decoration: underline;">👉 Ver Ficha de la Propiedad en la Web</a></p>` : ''}
      </div>
    ` : '';
    ...
    const adminHtmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
        ...
        <div style="padding: 24px;">
          <p style="font-size: 15px; margin-top: 0;"><strong>Origen:</strong> ${
            source === 'vender_propiedad' ? 'Solicitud Vender/Arrendar Propiedad' : source === 'property_inquiry' ? 'Consulta Ficha de Propiedad' : source === 'whatsapp' ? 'Botón de WhatsApp' : 'Formulario de Contacto Web'
          }</p>
          ${propertyInfoHtml}
          ...
          <p style="margin: 16px 0 8px 0;"><strong>Mensaje / Detalles de la consulta:</strong></p>
    ...
    let leadError = null;
    try {
      const leadSubject = propertyCode
        ? `¡Gracias por tu interés en la propiedad ${propertyCode}! - ${siteName}`
        : `¡Gracias por contactar a ${siteName}! Hemos recibido tu mensaje`;
      ...
      const leadHtmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
          ...
          <div style="padding: 24px; line-height: 1.6;">
            ...
            <p>Gracias por tu interés. Hemos recibido tu consulta y un ejecutivo inmobiliario revisará tu mensaje para responder a la brevedad.</p>
            
            <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 6px; padding: 16px; margin: 20px 0;">
              <p style="margin: 0; color: #166534; font-weight: bold; font-size: 14px;">✓ Solicitud registrada correctamente.</p>
            </div>
